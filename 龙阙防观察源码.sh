...
# ========== 远程强制更新（稳健版，无 timeout 依赖） ==========
LOCAL_VER="2.6"
UPDATE_RAW_BASE="https://raw.githubusercontent.com/wlbft996655/bafeite_update/main"
# 代理 URL 注意格式：ghproxy.com/ + 去协议后的域名路径
UPDATE_PROXY_BASE="https://ghproxy.com/raw.githubusercontent.com/wlbft996655/bafeite_update/main"

# ---------- 工具函数：安全下载（返回 HTTP 状态码） ----------
safe_download() {
    local url="$1"
    local output="$2"
    # 仅依赖 curl 内置超时，不依赖 timeout 命令
...
echo "正在检测更新..."
# 通道1：直连
REMOTE_VER=$(safe_download "${UPDATE_RAW_BASE}/version.txt" "/dev/null" | tr -d '\n\r ')
# 通道2：代理
if [ -z "$REMOTE_VER" ] || [ "$REMOTE_VER" != "${REMOTE_VER#*[0-9]*.[0-9]*}" ]; then
    : # 直连成功
else
    REMOTE_VER=$(safe_download "${UPDATE_PROXY_BASE}/version.txt" "/dev/null" | tr -d '\n\r ')
fi
...
if [ -z "$REMOTE_VER" ]; then
    echo "⚠️ 无法检测到远程版本，使用本地版本 ($LOCAL_VER)"
elif [ "$REMOTE_VER" = "$LOCAL_VER" ]; then
    echo "✅ 已是最新版本 ($LOCAL_VER)"
else
    echo "⚠️ 发现新版本 ($REMOTE_VER)，开始下载更新..."
    TMP_FILE="/data/local/tmp/龙阙防观察_new.sh"
    DOWNLOAD_SUCCESS=0
    
    # 下载脚本（直连）
    HTTP_CODE=$(safe_download "${UPDATE_RAW_BASE}/龙阙防观察.sh" "$TMP_FILE")
    if [ "$HTTP_CODE" = "200" ] && head -n1 "$TMP_FILE" | grep -q '#!'; then
        DOWNLOAD_SUCCESS=1
    else
        # 尝试代理
        HTTP_CODE=$(safe_download "${UPDATE_PROXY_BASE}/龙阙防观察.sh" "$TMP_FILE")
        if [ "$HTTP_CODE" = "200" ] && head -n1 "$TMP_FILE" | grep -q '#!'; then
            DOWNLOAD_SUCCESS=1
        fi
    fi
    ...
    if [ "$DOWNLOAD_SUCCESS" -eq 1 ]; then
        ...
        # 使用 cat 覆盖比 cp 更可靠（避免正在运行的脚本被占用问题）
        cat "$TMP_FILE" > "$SCRIPT_PATH" 2>/dev/null
        if [ $? -eq 0 ]; then
        ...
        echo "❌ 下载失败（HTTP状态码：${HTTP_CODE:-无}），继续使用旧版"
        rm -f "$TMP_FILE"
    fi
fi
# ========== 强制更新结束 ==========
...
progress_bar $current_mod $total_mod
printf "\n"
success "[17/27] 动态行为检测+线程名轮换+负载自适应调度"
...
printf "\n"
printf "${C_BLUE}╔═══════════════════════════════════════════════════╗${C_NC}\n"
printf "${C_BLUE}║${C_CYAN}                  龙阙防观察系统                  ${C_BLUE}║${C_NC}\n"
...
printf "${C_BLUE}║${C_WHITE}  功能概述：${C_GREEN}防观察/降低行为概率 ${C_BLUE}║${C_NC}\n"
