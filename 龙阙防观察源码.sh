...
# ========== 远程强制更新（终极稳健版） ==========
LOCAL_VER="2.6"
RAW_URL="https://raw.githubusercontent.com/wlbft996655/bafeite_update/main"
PROXY_URL="https://ghproxy.com/raw.githubusercontent.com/wlbft996655/bafeite_update/main"

# 安全下载函数：返回去除空白后的内容，失败返回空
download_text() {
    local url="$1"
    # 使用 -m 超时，-s 静默，-L 跟随重定向
    curl -sL -m 5 "$url" 2>/dev/null | tr -d '\r\n\t '
}

# 下载脚本文件：返回 HTTP 状态码，文件保存到 $2
download_file() {
    local url="$1"
    local out="$2"
    curl -sL -m 10 -w "%{http_code}" -o "$out" "$url" 2>/dev/null
}

echo "正在检查更新..."

# 获取远程版本号
REMOTE_VER=$(download_text "${RAW_URL}/version.txt")
[ -z "$REMOTE_VER" ] && REMOTE_VER=$(download_text "${PROXY_URL}/version.txt")

# 清理可能存在的非数字点字符（如 v2.7 → 2.7）
REMOTE_VER=$(echo "$REMOTE_VER" | grep -oE '[0-9]+\.[0-9]+' | head -1)

if [ -z "$REMOTE_VER" ]; then
    echo "⚠️ 无法获取远程版本，继续使用本地版本 ($LOCAL_VER)"
elif [ "$REMOTE_VER" = "$LOCAL_VER" ]; then
    echo "✅ 已是最新版本 ($LOCAL_VER)"
else
    echo "⚠️ 发现新版本 ($REMOTE_VER)，正在更新..."
    TMP_FILE="/data/local/tmp/.longque_update_tmp.sh"
    HTTP_CODE=""
    
    # 尝试直连下载
    HTTP_CODE=$(download_file "${RAW_URL}/龙阙防观察.sh" "$TMP_FILE")
    if [ "$HTTP_CODE" != "200" ] || ! head -1 "$TMP_FILE" | grep -q '#!'; then
        # 尝试代理下载
        HTTP_CODE=$(download_file "${PROXY_URL}/龙阙防观察.sh" "$TMP_FILE")
    fi
    ...
    if [ "$HTTP_CODE" = "200" ] && head -1 "$TMP_FILE" | grep -q '#!'; then
        ...
        echo "❌ 下载新版本失败（HTTP状态码：${HTTP_CODE:-无}），使用旧版本。"
        rm -f "$TMP_FILE"
    fi
fi
# ========== 强制更新结束 ==========
