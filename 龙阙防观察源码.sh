...
# ========== 远程强制更新（调试版） ==========
LOCAL_VER="2.6"
RAW_BASE="https://raw.githubusercontent.com/wlbft996655/bafeite_update/main"
PROXY_BASE="https://ghproxy.com/raw.githubusercontent.com/wlbft996655/bafeite_update/main"

echo ">>> [更新] 本地版本: $LOCAL_VER"

# 获取远程版本
REMOTE_VER=""
echo ">>> [更新] 正在直连获取版本..."
REMOTE_VER=$(curl -sL --connect-timeout 3 -m 5 "${RAW_BASE}/version.txt" 2>/dev/null | tr -d '\r\n\t ')
echo ">>> [更新] 直连结果: '$REMOTE_VER'"

if [ -z "$REMOTE_VER" ]; then
    echo ">>> [更新] 直连失败，尝试代理..."
    REMOTE_VER=$(curl -sL --connect-timeout 3 -m 5 "${PROXY_BASE}/version.txt" 2>/dev/null | tr -d '\r\n\t ')
    echo ">>> [更新] 代理结果: '$REMOTE_VER'"
fi

if [ -z "$REMOTE_VER" ]; then
    echo ">>> [更新] 无法获取远程版本，终止更新。"
elif [ "$REMOTE_VER" = "$LOCAL_VER" ]; then
    echo ">>> [更新] 版本相同，无需更新。"
else
    echo ">>> [更新] 发现新版本: $REMOTE_VER，开始下载..."
    TMP_FILE="/data/local/tmp/龙阙防观察_new.sh"
    DOWNLOAD_OK=0
    
    for url in "${RAW_BASE}/龙阙防观察.sh" "${PROXY_BASE}/龙阙防观察.sh"; do
        echo ">>> [更新] 尝试下载: $url"
        HTTP_CODE=$(curl -sL --connect-timeout 3 -m 10 -w "%{http_code}" -o "$TMP_FILE" "$url" 2>/dev/null)
        echo ">>> [更新] 返回状态码: $HTTP_CODE"
        if [ "$HTTP_CODE" = "200" ] && [ -s "$TMP_FILE" ] && head -1 "$TMP_FILE" | grep -q '#!'; then
            DOWNLOAD_OK=1
            echo ">>> [更新] 下载成功，文件有效。"
            break
        else
            echo ">>> [更新] 下载无效或失败，尝试下一个地址。"
        fi
    done
    ...
    if [ $DOWNLOAD_OK -eq 1 ]; then
        ...
        echo ">>> [更新] 所有下载地址均失败，更新放弃。"
    fi
    rm -f "$TMP_FILE"
fi
echo ">>> [更新] 更新流程结束，继续执行主程序。"
# ========== 更新结束 ==========
# ========== 更新结束 ==========
