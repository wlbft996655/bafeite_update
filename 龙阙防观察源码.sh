...
get_remote_ver() {
    local url="$1"
    curl -sL --connect-timeout 3 -m 5 "$url" 2>/dev/null | tr -d '\r\n\t '
}

echo "检查更新中..."
REMOTE_VER=$(get_remote_ver "${RAW_BASE}/version.txt")
[ -z "$REMOTE_VER" ] && REMOTE_VER=$(get_remote_ver "${PROXY_BASE}/version.txt")
...
if [ -n "$REMOTE_VER" ] && [ "$REMOTE_VER" != "$LOCAL_VER" ]; then
    ...
    for url in "${RAW_BASE}/龙阙防观察.sh" "${PROXY_BASE}/龙阙防观察.sh"; do
        curl -sL --connect-timeout 3 -m 10 -o "$TMP_FILE" "$url" 2>/dev/null
        if [ -f "$TMP_FILE" ] && head -1 "$TMP_FILE" | grep -q '#!'; then
    ...
    echo "⚠️ 无法检测更新，继续使用本地版本 ($LOCAL_VER)"
else
