...
# ========== 远程更新（版本比较，失败不影响主流程） ==========
LOCAL_VER="2.6"
UPDATE_URL_BASE="https://raw.githubusercontent.com/wlbft996655/bafeite_update/main"
PROXY_URL="https://ghproxy.com/"

# 获取远程版本号（仅尝试1次，2秒超时）
REMOTE_VER=""
REMOTE_VER=$(timeout 3 curl -s --connect-timeout 2 --max-time 3 \
            "${PROXY_URL}${UPDATE_URL_BASE}/version.txt" 2>/dev/null)

# 校验版本号格式
case "$REMOTE_VER" in
    [0-9]*.[0-9]*) ;;   # 合法
    *) REMOTE_VER="" ;; # 非法或超时，视为获取失败
esac
...
if [ -z "$REMOTE_VER" ] || [ "$REMOTE_VER" = "$LOCAL_VER" ]; then
    ...
    echo "⚠️ 发现新版本 ($REMOTE_VER)，开始更新..."
    TMP_FILE="/data/local/tmp/巴菲特防观察_new.sh"
    # 下载新脚本（限制4秒）
    timeout 4 curl -sL -w "%{http_code}" -o "$TMP_FILE" \
        "${PROXY_URL}${UPDATE_URL_BASE}/巴菲特防观察.sh" \
        --connect-timeout 2 --max-time 3 > /tmp/curl_code.$$ 2>/dev/null
    HTTP_CODE=$(cat /tmp/curl_code.$$ 2>/dev/null)
    rm -f /tmp/curl_code.$$
    ...
    if [ "$HTTP_CODE" = "200" ] && head -n1 "$TMP_FILE" | grep -q '#!'; then
        ...
        rm -f "$TMP_FILE"
        echo "✅ 更新完成！备份为 ${SCRIPT_PATH}.bak，请重新运行脚本"
        exit 0   # 自身被替换，必须退出
    else
        echo "❌ 更新失败（HTTP $HTTP_CODE），继续使用当前版本"
        rm -f "$TMP_FILE"
    fi
fi
